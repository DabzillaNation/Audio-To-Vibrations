...
async fn run_vibration_logic(settings: Arc<Mutex<AppSettings>>, audio_device: Device) -> Result<()> {
    ...
    // Safely unwrap device name to avoid crashing the setup process
    let dev_name = audio_device.name().unwrap_or_else(|_| String::from("Unknown Audio Device"));
    println!("[Audio] Using audio device: {}", dev_name);
    ...
    'reconnection_loop: loop {
        ...
        // Use if let Err instead of `?` so the background thread doesn't crash if the server disconnects immediately.
        if let Err(e) = client.start_scanning().await {
            eprintln!("[Buttplug] Failed to start scanning: {}. Retrying connection...", e);
        ...
        if let Err(e) = client.stop_scanning().await {
            eprintln!("[Buttplug] Failed to stop scanning: {}. Retrying connection...", e);
            continue 'reconnection_loop;
        }
        ...
        loop {
            ...
            // We use a timeout to prevent hanging forever if the audio stream pauses.
            // This ensures we keep evaluating connection/device status periodically.
            match time::timeout(Duration::from_millis(100), rx.recv_many(&mut collected_values, SAMPLE_LIMIT)).await {
                Ok(0) => {
                ...
                Ok(_) => {
                    // Data received normally from the audio thread.
                }
                Err(_) => {
                    // Timeout occurred (no audio data received). 
                    // Push a 0.0 value as a heartbeat to ensure we still send a command
                    // to the device. This allows us to instantly detect if Intiface disconnected.
                    collected_values.push(0.0);
                }
            }
            ...
            // If the devices list is empty (e.g. disconnected silently), break immediately.
            if all_connected.is_empty() {
                println!("[Buttplug] No devices connected. Triggering re-scan/reconnect.");
                break;
            }
            ...
            // Even if the server connection (Intiface) is alive, a command failure usually
            // means the specific device is gone. We break the loop to re-scan.
            if command_failed {
                eprintln!("[Buttplug] Command failed. Connection to device lost. Restarting loop...");
